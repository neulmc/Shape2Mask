...
class UpsamplingDecoderLayer(nn.Module):
    ...
    def __init__(self, model_dimension, base_control_points, max_control_points, inner):
        ...
        self.register_buffer("point_embedding",
                             point_encoding(self.model_dimension * 2, max_len=self.max_control_points))
        self.number_iterations = int(math.log2(self.max_control_points // self.base_control_points)) + 1
        print("{0} to {1} over {2} layers".format(self.base_control_points, self.max_control_points,
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index,
                src_padding_mask=None, lid=0, cls_token=None, reference_boxes=None):
        ...
        if lid == 0:
            return self.inner(tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index,
                              src_padding_mask, cls_token=cls_token, reference_boxes=reference_boxes)
        ...
        iteration_idx = lid
        insert_query_pos, insert_tgt = torch.split(self.point_embedding[self.idxs[iteration_idx]], self.model_dimension,
                                                   dim=1)
        ...
        reference_points = torch.stack((reference_points, insert_reference_points), dim=2).view(batch_size, -1,
        ...
        return \
        self.inner(tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask,
                   cls_token=cls_token, reference_boxes=reference_boxes)[0], (tgt, query_pos, insert_reference_points)
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, predict_in_box_space=False):
        ...
        self.layers = _get_clones(decoder_layer, num_layers)  # clone a decoder layer the num_layer times,
        self.num_layers = num_layers
        self.return_intermediate = return_intermediate
        self.predict_in_box_space = predict_in_box_space

        # iterative refinement.
        self.xy_embed = None  # yr: 这里虽然是 None, 实例化之后，会定义一个 动态属性
