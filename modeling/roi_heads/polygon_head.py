...
from modeling.rawtransformer import DeformableTransformerDecoder, DeformableTransformerControlLayer, MLP, \
...
from modeling.box_supervisor.rawbox_supervisor import BoxSupLoss, create_box_targets, BoxSupervisor
        ...
        if self.training:
            ...
            # pred in mask scale, like P3, P4
                ...
                pred_polygons = [output_coords * pad_img_wh  # NOTE: Does minusing offset matter?
                                 for lid, output_coords in
                                 enumerate(outputs_coords)]  # List(Tensor, ...) Tensor shape=(N_inst, Np, 2)
            #sss = pred_masks[0].detach().cpu().numpy()
            #ss1 = sss[0,0]
