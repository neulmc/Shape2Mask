    ...
    def __call__(self, images, preds, instances):
        ...
        if self.enable_local_pairwise_loss:
            ...
            if storage.iter - self.pairwise_cold_iters > 0:
                pairwise_warmup_factor = min(
                    (storage.iter - self.pairwise_cold_iters) / float(self.pairwise_warmup_iters), 1.0)
            else:
...
class BoxSupLoss():
    ...
    def loss_points_proj(self, pred_masks, targets, num_masks, **kwargs):
        ...
        # points proj
        proj_boxes = torch.cat([pred_polys.min(dim=1)[0], pred_polys.max(dim=1)[0]],
                               dim=-1)  # shape=(N, 4), x1y1x2y2 format
        # ciou loss
    ...
    def loss_projection(self, pred_masks, targets, num_masks, **kwargs):
        ...
        if "dice" in self.loss_projection_type:
            ...
            loss.update({"loss_proj_dice": (mask_losses_dice_x + mask_losses_dice_y) * self.loss_weights.get(
                'loss_proj_dice', 0.)})
        if "ce" in self.loss_projection_type:
            ...
            loss.update(
                {"loss_proj_ce": (mask_losses_ce_x + mask_losses_ce_y) * self.loss_weights.get('loss_proj_ce', 0.)})
        del target_masks
        return loss
    ...
    def loss_avg_projection(self, pred_masks, targets, num_masks, **kwargs):
        ...
        if "dice" in self.loss_projection_type:
            mask_losses_dice_y = dice_loss_jit(pred_proj_y, tgt_proj_y, num_masks)
            mask_losses_dice_x = dice_loss_jit(pred_proj_x, tgt_proj_x, num_masks)
            loss.update({"loss_avg_proj_dice": (mask_losses_dice_x + mask_losses_dice_y) * self.loss_weights.get(
                'loss_avg_proj_dice', 0.)})
        if "ce" in self.loss_projection_type:
            mask_losses_ce_y = sigmoid_ce_loss_jit(pred_proj_y, tgt_proj_y, num_masks)
            mask_losses_ce_x = sigmoid_ce_loss_jit(pred_proj_x, tgt_proj_x, num_masks)
            loss.update({"loss_avg_proj_ce": (mask_losses_ce_x + mask_losses_ce_y) * self.loss_weights.get(
                'loss_avg_proj_ce', 0.)})
        return loss
...
def get_region_level_energy(imgs, masks, num_channel):
    ...
    avg_sim = torch.sum(imgs * masks, dim=(2, 3), keepdim=True) / torch.sum(masks, dim=(2, 3), keepdim=True).clamp(
        min=1e-5)
    # shape=(N, C, 1, 1)
