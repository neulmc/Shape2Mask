...
def create_box_targets_crop(images, images_norm, instances, mask_stride, kernel_size=3, dilation=1, crop_size=64, mask_padding_size=4, sigma=2.0):
    ...
    #clipped_per_bitmasks[int(_box[0]): int(_box[3])+1, int(_box[1]): int(_box[2])+1] = 1.0 # 我觉得多了一列
    clipped_per_bitmasks[int(_box[0]): int(_box[3]), int(_box[1]): int(_box[2])] = 1.0 # lmc new
    gt_boxmasks = clipped_per_bitmasks.unsqueeze(0).expand(gt_imgs.size(0), -1, -1)
    return gt_boxmasks, gt_imgs_sim, gt_imgs, gt_boxes


@torch.no_grad()
def create_mask_targets_crop(images, images_norm, instances, mask_stride, kernel_size=3, dilation=1, crop_size=64,
                            mask_padding_size=4, sigma=2.0):
    """
    instances ([Instance(), ...])
    clip_boxes (Tensor()) :
    return:
    """
    img_h, img_w = images.shape[-2:]
    gt_boxmasks, gt_imgs_sim, gt_imgs, gt_realmasks = [], [], [], []
    start = int(mask_stride // 2)
    clipped_size = tuple([int(crop_size + 2 * mask_padding_size)] * 2)
    ...
    for idx, per_img_inst in enumerate(instances):
        ...
        # padding mask
        offset = mask_padding_size * (per_img_gt_boxes[:, 2:] - per_img_gt_boxes[:, :2]) / float(
            crop_size)  # shape=(N, 2)
        per_img_enlarged_boxes = per_img_gt_boxes + torch.cat([-offset, offset], dim=-1)  # shape=(N, 4)
        # box maybe out of the image, so we pad the image and add a bias to the enlarged box
        pad_size = int(torch.max(offset).item() + 1)
        ...
        ###############  lmc added sup ######################
        gt_bitmasks = []
        for polygons in per_img_inst.gt_masks.polygons:
            mask = polygons_to_bitmask(polygons, img_h, img_w)
            #sss = mask.astype(float)
            gt_bitmasks.append(mask)
        gt_bitmasks = torch.from_numpy(np.stack(gt_bitmasks)).to(images.device)  # (N, H, W)

        #s1, p1, b1 = gt_bitmasks.float().detach().cpu().numpy()[0], per_img_inst.gt_masks.polygons[0], per_img_gt_boxes.detach().cpu().numpy()[0]
        #s2, p2, b2 = gt_bitmasks.float().detach().cpu().numpy()[1], per_img_inst.gt_masks.polygons[1], per_img_gt_boxes.detach().cpu().numpy()[1]
        #s3, p3, b3 = gt_bitmasks.float().detach().cpu().numpy()[2], per_img_inst.gt_masks.polygons[2], per_img_gt_boxes.detach().cpu().numpy()[2]
        #s4, p4, b4 = gt_bitmasks.float().detach().cpu().numpy()[3], per_img_inst.gt_masks.polygons[3], per_img_gt_boxes.detach().cpu().numpy()[3]
        #s5, p5, b5 = gt_bitmasks.float().detach().cpu().numpy()[4], per_img_inst.gt_masks.polygons[4], per_img_gt_boxes.detach().cpu().numpy()[4]

        gt_bitmasks_padded = F.pad(gt_bitmasks, (pad_size, pad_size, pad_size, pad_size), value=0.)


        batch_inds = torch.zeros(len(per_img_enlarged_boxes)).type_as(per_img_enlarged_boxes)[:, None]
        batch_inds_gt = torch.arange(len(per_img_enlarged_boxes), device=per_img_enlarged_boxes.device)[:, None]
        rois = torch.cat([batch_inds, per_img_enlarged_boxes], dim=1)
        rois_gt = torch.cat([batch_inds_gt, per_img_enlarged_boxes], dim=1)

        ###############  lmc modify sup ######################
        clipped_per_img_mask = ROIAlign(clipped_size, 1.0, 0, aligned=True).forward(
            padded_mask[None, None], rois)  # (N, 1, H, W)
        clipped_per_img_realmask = ROIAlign(clipped_size, 1.0, 0, aligned=True).forward(
            gt_bitmasks_padded.float().unsqueeze(1), rois_gt)  # 输出 (N, 1, H, W)

        #ssss = rois.detach().cpu().numpy()
        #pp1, pp2 = gt_bitmasks_padded.float().detach().cpu().numpy()[0], gt_bitmasks_padded.float().detach().cpu().numpy()[1]
        #cc1, cc2 = clipped_per_img_realmask.float().detach().cpu().numpy()[0,0], clipped_per_img_realmask.float().detach().cpu().numpy()[1,0]
        #ssss = clipped_per_img_realmask[0,0,:,:].detach().cpu().numpy()
        gt_realmasks.append(clipped_per_img_realmask)

        # prepare image similarity
        clipped_per_img = ROIAlign(clipped_size, 1.0, 0, aligned=True).forward(
            img_lab_padded.unsqueeze(0), rois).squeeze(1)  # shape=(N, C, clipped_size, clipped_size)
        gt_img_sim = get_images_color_similarity(clipped_per_img, clipped_per_img_mask, kernel_size=kernel_size,
                                                 dilation=dilation, sigma=sigma)
        gt_imgs_sim.append(gt_img_sim)
        # prepare image norm for level set
        clipped_per_img_norm = (
            ROIAlign(clipped_size, 1.0, 0, aligned=True)
    ...
    gt_imgs = torch.cat(gt_imgs)
    gt_boxes = torch.cat(gt_boxes)
    gt_realmasks = torch.cat(gt_realmasks)

    #gt_realmasks_1 = gt_realmasks.detach().cpu().numpy()[0, 0, :, :]
    #gt_realmasks_ = gt_realmasks.detach().cpu().numpy()[1, 0, :, :]
    ...
    clipped_per_bitmasks[int(_box[0]): int(_box[3]), int(_box[1]): int(_box[2])] = 1.0  # lmc new
    gt_boxmasks = clipped_per_bitmasks.unsqueeze(0).expand(gt_imgs.size(0), -1, -1)
    return gt_boxmasks, gt_imgs_sim, gt_imgs, gt_boxes, gt_realmasks
