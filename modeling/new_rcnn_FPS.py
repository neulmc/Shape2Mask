...
__all__ = ["NewGeneralizedRCNN","supNewGeneralizedRCNN"]
...
class NewGeneralizedRCNN(GeneralizedRCNN):
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer('pixel_std_level_set', torch.tensor(pixel_std_bgr).view(1, -1, 1, 1)) # (1, 3, 1, 1) 3 for channels
        #self.pixel_std_level_set = torch.tensor(pixel_std_bgr).view(1, -1, 1, 1).to(self.device)
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        #sss = self.backbone.fpn_lateral2.weight
        #ss1 = self.roi_heads.mask_head.decoder.layers[0].linear1.weight
        ...
        _, detector_losses, shape_point_weights = self.roi_heads(images_info, features, proposals, gt_instances)
        ...
        storage = get_event_storage()
        if not shape_point_weights is None:
            shape_point_weights = torch.mean(shape_point_weights, dim=(0, 1))
            for i in range(shape_point_weights.shape[0]):
                storage.put_scalar("[metric]-" + str(i), shape_point_weights[i])
    ...
    def inference(
        ...
        if detected_instances is None:
            ...
            results, _, shape_point_weights = self.roi_heads(None, features, proposals, None)
        else:
            detected_instances = [x.to(self.device) for x in detected_instances]
            results, shape_point_weights = self.roi_heads.forward_with_given_boxes(features, detected_instances)

        if not shape_point_weights is None:
            shape_point_weights = torch.mean(shape_point_weights, dim=(1))
            results[0].shape_point_weights = shape_point_weights
        #do_postprocess = False # revised
        if do_postprocess:
            start_time = time.time()
            assert not torch.jit.is_scripting(), "Scripting is not supported for postprocess."
            post_result = NewGeneralizedRCNN._postprocess(results, batched_inputs, images.image_sizes,
                                            self.roi_heads.mask_head.decoder.num_layers + self.roi_heads.mask_head.shape_decode.num_layers)
            end_time = time.time()
            return post_result, end_time-start_time
        else:
            return results

    @staticmethod
    def _postprocess(instances, batched_inputs: List[Dict[str, torch.Tensor]], image_sizes, head_nums):
        """
        ...
        for results_per_image, input_per_image, image_size in zip(
            ...
            r = detector_postprocess(results_per_image, height, width, head_nums)
...
@META_ARCH_REGISTRY.register()
class supNewGeneralizedRCNN(GeneralizedRCNN):
    """
    Generalized R-CNN. Any models that contains the following three components:
    1. Per-image feature extraction (aka backbone)
    2. Region proposal generation
    ...
    def __init__(
        ...
        Args:
            ...
            vis_period: the period to run visualization. Set to 0 to disable.
        """
        super(supNewGeneralizedRCNN, self).__init__(backbone=backbone, proposal_generator=proposal_generator,
                                                 roi_heads=roi_heads,
        ...
        self.register_buffer('pixel_std_level_set',
    ...
    def inference(
        ...
        if do_postprocess:
            assert not torch.jit.is_scripting(), "Scripting is not supported for postprocess."
            return supNewGeneralizedRCNN._postprocess(results, batched_inputs, images.image_sizes,
                                                   self.roi_heads.mask_head.decoder.num_layers + self.roi_heads.mask_head.shape_decode.num_layers)
        else:
            return results
    ...
    def get_lab_images(self, images_info, original_images, is_rgb=False):
        ...
        for index in range(len(original_images)):
            ...
            image_lab = color.rgb2lab(original_images[index].byte().permute(1, 2, 0).cpu().numpy())
            image_lab = torch.as_tensor(image_lab, device=original_images[index].device,
                                        dtype=torch.float32)  # fp16的时候怎么办？
            image_lab = image_lab.permute(2, 0, 1)[None].contiguous()
            images_lab.append(image_lab)
        images_info['images'] = torch.cat(images_lab)
